...
We have worked with RxJS Observables in three ways so far:

1. We have [tested the getCount method of CounterService](#testing-a-service-with-internal-state). The spec subscribes to the returned Observable. On subscription, the Observable emits a “next” value _synchronously_. We were able to apply simple pattern to check the emitted value:
...
2. We have [tested the TranslatePipe](#translatepipe-test) and have replaced the `TranslatePipe` with a fake. To simulate the fetching of translations, the Observable emits a “next” value _asynchronously_ with a delay.
   In the spec, we have used `fakeAsync` and `tick` to simulate the passage of time.
   ...
   it('…', fakeAsync(() => {
     ...
     tick(100); // Insert appropriate delay
     expect(content).toBe(expectedValue);
   }));
   ```

These patterns cover most cases in which an Observable emits one “next” value and completes thereafter, or instantly fails with an error.

There are Observables that implement a more complex logic. They consume other Observables as input and emit multiple values. You can find them in Components or Services most of the time.

<aside class="margin-note">Create Observables</aside>

First, let us have a look at different ways to create Observables in tests.

To create a Observable to be used as a fake, we have used RxJS’ `of` function. This Observable emits one “next” value and completes.

```typescript
of(123);
```

We can manipulate the Observable by applying RxJS operators, such as `delay`.

```typescript
of(123).pipe(delay(100));
```

A related way to create an Observable is the `from` function. This Observable emits a sequence of “next” values and completes. The `from` function supports arrays and other iterables as well as Promises.

```typescript
from([1, 2, 3, 4, 5]);
```

Note that the values are emitted one after another without delay. If you want to spread out the values with a delay

```typescript
from([1, 2, 3, 4, 5]).pipe(concatMap((x) => of(x).pipe(delay(1000))));
```
